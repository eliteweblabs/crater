...
// Body (all fields optional): { "name", "description", "quantity", "price" }
// price is whole dollars (stored as cents), matching add_invoice_items.
Route::put('/custom/invoice/{invoiceId}/items/{itemId}', function (Illuminate\Http\Request $request, $invoiceId, $itemId) {
    ...
    $item = InvoiceItem::where('invoice_id', $invoice->id)->where('id', $itemId)->first();
    if (!$item) {
    ...
    if (isset($validated['price'])) {
        ...
        $item->base_price = (int) round($item->price * $exchangeRate);
        $totalsChanged = true;
    }
    if ($totalsChanged) {
        $item->total = (int) round(((int) $item->price) * (float) $item->quantity);
        $item->base_total = (int) round($item->total * $exchangeRate);
    }
    ...
    return response()->json([
        ...
        'price'          => round(((int) $item->price) / 100, 2),
        'total'          => round(((int) $item->total) / 100, 2),
        'invoice_total'  => round(((int) $invoice->fresh()->total) / 100, 2),
        'admin_url'      => url("/admin/invoices/{$invoice->id}/view"),
    ]);
});
